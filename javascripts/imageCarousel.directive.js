  ...
  angular
    ...
    .directive('imageCarousel', ['$timeout',function($timeout) {
      return {
        ...
        '       >' +
        '           <div class="image_carousel__item_content" ng-if="animation.armed && animation.reverse || !animation.reverse && animation.active || !animation.armed && animation.active && item.index == 1 || !animation.armed && animation.reverse && !animation.active && item.index == -1 || !animation.armed && item.index == 0 || animation.armed && !animation.reverse && item.index == -1" image-carousel-item get-content="getItemContent"></div>' +
        '       <div>' +
        '   </div>' +
        '</div>'
      }
